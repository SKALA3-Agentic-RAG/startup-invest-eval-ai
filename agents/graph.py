...
def route_after_search(state: GraphState) -> Literal["startup_eval", "decision"]:
    """Skip evaluation when no candidates remain."""
    startups = state.get("startups") or []
    if not startups:
        logger.info("route_after_search -> decision (no startups)")
        return "decision"
...
def build_graph(checkpointer):
    ...
    Flow: macro → search → startup_eval (tech ∥ market per startup) → decision →
    report → END.
    """
    ...
    builder.add_conditional_edges(
        "search",
        route_after_search,
        {"startup_eval": "startup_eval", "decision": "decision"},
    )
    builder.add_edge("startup_eval", "decision")
